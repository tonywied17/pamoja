...
- `pamoja-sim` - hardware-free device simulators behind the core `Sensor` and `Actuator` traits: a fake sensor with seedable noise and drift, a replay sensor for scripted readings, and a recording actuator, so the full stack runs and is tested with no hardware.
