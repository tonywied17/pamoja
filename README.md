...
- `pamoja-lorawan` - a `no_std`, allocation-free LoRaWAN 1.0.x MAC framing layer: build and parse data-frame `PHYPayload`s with the AES-CMAC message integrity code and the AES payload encryption the spec mandates, binding the device address and frame counter into both, validated against the AES (FIPS-197) and CMAC (RFC 4493) reference vectors. The secured-packet half that turns the LoRa link budget into real over-the-air LoRaWAN traffic, ahead of the radio driver.
